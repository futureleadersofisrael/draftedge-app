...
export default function ParentIntroPage() {
  ...
  return (
    <>
      ...
      <div
        dir="rtl"
        className={`min-h-screen bg-[#F8FAF8] text-[#1C1C1C] relative ${inter.className}`}
      >
        <div className="flex items-center justify-center min-h-screen p-4 md:p-8">
        <main className="relative z-10 w-full max-w-2xl bg-white rounded-xl px-8 py-16 md:px-12 md:py-20 text-center border border-black/[0.05] shadow-[0_4px_6px_-1px_rgba(0,0,0,0.05),0_2px_4px_-1px_rgba(0,0,0,0.03)]">
          ...
          <div className="flex flex-col items-center gap-6">
            <button
              type="button"
              onClick={() => router.push("/parent-step-1")}
            ...
            >
              <span className="relative font-bold tracking-wide text-lg flex items-center justify-center gap-4">
                המשך
        ...
        <div className="fixed bottom-8 right-8 hidden md:block opacity-40">
          ...
          <div className="text-[9px] text-[#6B7280] uppercase tracking-widest mt-1 flex items-center gap-1">
            <span className="w-1 h-1 bg-[#6B7280] rounded-full" /> Encrypted
          </div>
        </div>
      </div>
    </>
